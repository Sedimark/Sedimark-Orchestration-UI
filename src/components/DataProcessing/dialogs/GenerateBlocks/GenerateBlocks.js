...
export default function GenerateBlocks(props) {
    ...
    const blockResultsRef = useRef(false); // Use ref instead of local variable
    ...
    const [generatedBlockName, setGeneratedBlockName] = useState("happy cameleon");
    const [generatedBlockType, setGeneratedBlockType] = useState("transformer");
    const [thereWasAnError, setThereWasAnError] = useState(false);
    const [blockCode, setBlockCode] = useState(`import pandas as pd
    ...
    const [generationMessage, setGenerationMessage] = useState("Fixing the code if there are any issues in the genration");
  ...
  useEffect(() => {
  const ws = new WebSocket(`${process.env.REACT_APP_MAGE_WS_URL}block/generate`);
  ...
  ws.onmessage = (event) => {
    ...
    if (message.generation_status === true) {
      ...
      dispatch(setStoredBlockIsGenerating(false));
      dispatch(setEditorValueBlockGenerating(message.generation));
      ...
      dispatch(setErrorWhileGenerating(false));
  ...
  ws.onclose = () => {
    ...
    setBlockIsGenerating(false);
    if (!blockResultsRef.current) {
      setThereWasAnError(true);
      dispatch(setErrorWhileGenerating(true));
    }
